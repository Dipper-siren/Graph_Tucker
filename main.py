...
class Experiment:
    ...
    def __init__(self, learning_rate=0.0005, ent_vec_dim=200, rel_vec_dim=200, 
                 ...
                 label_smoothing=0., loss_weight=2.5, regularization=1.0, bias=0.2):
    ...
    def get_graph_E(self, data_idxs, model):
        er_vocab = self.get_graph_vocab(data_idxs)
        for i in range(len(d.entities)):
            result_er = er_vocab[i]      # 所有可以推导出尾实体的三元组的头实体和关系。
            if len(result_er) != 0:
                result_er = np.array(result_er)
                e1_idx = torch.tensor(result_er[:, 0])
                r_idx = torch.tensor(result_er[:, 1])
                if self.cuda:
                    e1_idx = e1_idx.cuda()
                    r_idx = r_idx.cuda()
    ...
    def get_two_rela_idxs(self, data_idxs):
        ...
        for i in range(1, len(data_idxs)+1):
    ...
    def get_graph_vocab(self, data):
        er_vocab = defaultdict(list)
        for triple in data:
            er_vocab[triple[2]].append((triple[0], triple[1]))
        return er_vocab
    ...
    def evaluate(self, model, data):
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            ...
            if self.cuda:
                ...
                r_idx = r_idx.cuda()
                e2_idx = e2_idx.cuda()
            predictions, margin_loss = model.forward(e1_idx, r_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--loss_weight", type=float, default=2.5, nargs="?",
